...
class Command(BaseCommand):
    ...
    # ===================================================================
    #  HOLDER BORROW SUCCESS
    #  - ticket: PENDING -> DANG_MUON
    ...
    def process_holder_borrow_success(self, tx_id: int):
        ...
        with db_transaction.atomic():
            ...
            if hasattr(h, "mon_truoc") and getattr(h, "mon_truoc", None) is None:
    ...
    # ===================================================================
    #  HOLDER RETURN SUCCESS
    #  - phiếu trả: PENDING -> SUCCESS
    #  - phiếu mượn đang mở: DANG_MUON -> DA_TRA
    #  - holder: -> san_sang
    #  - KHÔNG auto mòn; nếu phiếu trả có mon_sau thì apply vào holder.mon
    # ===================================================================
    def process_holder_return_success(self, tx_id: int):
        ...
        with db_transaction.atomic():
            ...
            holder.trang_thai_tai_san = "san_sang"
            ...
            if hasattr(history_borrow, "mon_truoc") and getattr(history_borrow, "mon_truoc", None) is None:
                history_borrow.mon_truoc = mon_truoc
            if hasattr(history_borrow, "mon_sau") and getattr(history_borrow, "mon_sau", None) is None:
            ...
            if hasattr(history_return, "mon_truoc") and getattr(history_return, "mon_truoc", None) is None:
                history_return.mon_truoc = mon_truoc
            # mon_sau: giữ đúng giá trị user nhập nếu có, không thì set = holder.mon hiện tại
            if hasattr(history_return, "mon_sau") and getattr(history_return, "mon_sau", None) is None:
            ...
            logger.info(
                f"[RETURN OK] Holder {holder.id} -> san_sang | "
                f"wear before={mon_truoc}, input={mon_nhap_tay}, final={holder.mon} | tx={tx_id}"
            )

    # ===================================================================
    #  TOOL SUCCESS – UPDATE TON KHO + TON_SAU
    # ===================================================================
